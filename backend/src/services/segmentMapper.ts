 ...
 */
export function assignPixelsToSegments(
  normalizedDsm: Float32Array,
  ...
  for (let row = 1; row < height - 1; row++) {
    for (let col = 1; col < width - 1; col++) {
      ...
      const dzdy = (
        -normalizedDsm[(row - 1) * width + (col - 1)] - 2 * normalizedDsm[(row - 1) * width + col]
        - normalizedDsm[(row - 1) * width + (col + 1)] + normalizedDsm[(row + 1) * width + (col - 1)]
        + 2 * normalizedDsm[(row + 1) * width + col] + normalizedDsm[(row + 1) * width + (col + 1)]
      ) / 8;
      ...
      for (let s = 0; s < segments.length; s++) {
        ...
        const da = Math.min(
          Math.abs(azimuth - segments[s].azimuthDegrees),
          360 - Math.abs(azimuth - segments[s].azimuthDegrees)
        );
