...
app.post('/api/roof-scan', async (req, res) => {
    ...
    try {
        ...
        const normalizedDsm = applyMaskAndNormalize(dsm.data, mask.data, dsm.width, dsm.height);
        // Step 5: Segment-to-pixel mapping
        console.log('5. Mapping pixels to segments...');
        const labels = assignPixelsToSegments(normalizedDsm, mask.data, segments, dsm.width, dsm.height);
        // Step 6: Polygon extraction
        console.log('6. Extracting facet polygons...');
        const facets = extractFacetPolygons(labels, segments, dsm.width, dsm.height, dsm.pixelScale, dsm.tiepoint);
