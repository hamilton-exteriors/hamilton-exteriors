 ...
 */
export function extractFacetPolygons(labels, segments, width, height, pixelScale, tiepoint) {
    return segments.map((seg, segId) => {
        const boundaryPoints = [];
        for (let row = 0; row < height; row++) {
            for (let col = 0; col < width; col++) {
                if (labels[row * width + col] !== segId)
                    continue;
        ...
        const polygon = hull?.geometry?.coordinates[0] || [];
        return createFacet(segId, seg, polygon);
    });
}
function createFacet(segId, seg, polygon) {
...
function degreesToRatio(degrees) {
 ...
 */
export function facetsToGeoJSON(facets) {
    return {
